...
import numpy as np
import pandas as pd
# import matplotlib.pyplot as plt # 不再需要绘图
...
def setup_env():
    ...
    sub_dirs = [
        'models',           # 存放 pth 模型
        'metrics',          # 存放指标和范围统计
        'cleaned_data',     # 存放清洗后的输入
        'prediction_data'   # 存放预测的轨迹坐标 CSV
    ]
...
def select_scientific_samples(X_scaled, X_original, n_select, result_dir):
    print(f"   正在执行科学采样 (K-Means, k={n_select})...")
    ...
    print(f"   已选出 {len(train_indices)} 个代表性样本用于训练")
    ...
    print(f"   ✅ 训练集物理参量范围已保存至: {save_path}")
...
def calculate_all_metrics(true_matrix, pred_matrix):
    ...
    for i in range(n):
        ...
        if (i+1) % 1000 == 0: print(f"   已处理 {i+1}/{n}")
...
if __name__ == "__main__":
    ...
    print(f"\n[7/7] ✅ 全部流程结束。")
    print(f"      - 模型: {res_dir}/models/kan_model_final.pth")
    print(f"      - 坐标: {res_dir}/prediction_data/")
    print(f"      - 指标: {res_dir}/metrics/")